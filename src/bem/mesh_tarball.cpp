...
const std::string mesh_tarball::MESH_DEFINITION_XML = "definition.xml";
const std::string mesh_tarball::MESH_DEFINITION_XML_ROOT_ELEMENT = "contents";
const std::string mesh_tarball::PDB_TAG = "pdb";
const std::string mesh_tarball::PQR_TAG = "pqr";
const std::string mesh_tarball::XYZQR_TAG = "xyzqr";
const std::string mesh_tarball::VERTEX_NORMALS_TAG = "vertex_normals";
const std::string mesh_tarball::MESH_TAG = "mesh";
const std::string mesh_tarball::ENERGIES_TAG = "energies";
const std::string mesh_tarball::FH_TAG = "fh";
const std::string mesh_tarball::TENSOR_TAG = "diffusion_tensor";
const std::string mesh_tarball::CENTRE_TAG = "centre";
const std::string mesh_tarball::ELLIPSOID_TAG = "ellipsoid";
const std::string mesh_tarball::ECM_TAG = "ecm";


mesh_tarball::mesh_tarball(const std::string& filename) {
    init(filename);
}

void mesh_tarball::init(const std::string& filename)
{
...
mesh_tarball::~mesh_tarball() {

    try {
        // delete the temporary folder
        for (boost::filesystem::directory_iterator itr(work_dir), end; itr != end; ++itr)
        {
            boost::filesystem::remove(*itr);
        }
        boost::filesystem::remove(work_dir);
...
int mesh_tarball::copy_data(struct archive *ar, struct archive *aw)
{
    int r;
    const void *buff;
    size_t size;
...
void mesh_tarball::extract(const std::string& filename)
{
    struct archive *a;
    struct archive *ext;
...
void mesh_tarball::parse_definition_xml()
{
    ...
    if (!loadOkay) {
        std::cerr << "ERROR: Failed to load " << MESH_DEFINITION_XML << " within mesh " << mtz_filename << std::endl;
        throw Mesh_TarBall_Exception();
    }

    std::string err_fname_path = MESH_DEFINITION_XML + " (" + mtz_filename + ")";
    ...
    // block: name
    {
        ...
        if (!pElem)
        {
            std::cerr << "ERROR: Unable to find root element in " << err_fname_path << std::endl;
            throw Mesh_TarBall_Exception();
        }
        std::string bem_config_name = pElem->Value();
        if (bem_config_name != MESH_DEFINITION_XML_ROOT_ELEMENT)
        {
            std::cerr << "ERROR: Root element should be: " << MESH_DEFINITION_XML_ROOT_ELEMENT << " in " << err_fname_path <<  std::endl;
            throw Mesh_TarBall_Exception();
        }
    ...
    // a triplet is a tuple of 3 values- the third value (a bool) is whether or not the tag is mandatory
    ...
    key_val_defs.push_back( triplet(&VERTEX_NORMALS_TAG, &vertex_normal_filename, false) );
    ...
    for (std::vector<triplet>::iterator it=key_val_defs.begin(), end=key_val_defs.end(); it != end; ++it)
    {
        triplet& def = *it;
        ...
        if (pElem) {
            ...
            if(pElem->NextSiblingElement( tag.c_str() )) {
                std::cerr << "ERROR: Multiple \"" << tag << "\" tags in " << err_fname_path << std::endl;
                failed = true;
            }
        ...
        else
        {
            ...
            // If it is mandatory then failing to find the tag is a fatal error -- cannot make a mesh
            // without this information. (e.g. gts/off file, charge locations)
            if (mandatory) {
                std::cerr << "ERROR: Failed to find mandatory tag \"" << tag << "\" in " << err_fname_path << std::endl;
                failed = true;
            }
        }
    }
