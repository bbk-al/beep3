...
public:
    ...
    void insert(std::vector<Charge> insertions) {
        ...
        if (super::size() == expect) {
            super_charm::thisProxy[CkMyNode()].finalize(stashed_cb);
        }
    }
    ...
    void finalize(CkCallback cb)
    {
        ...
        int erased=0;
#if 1
        int num_pts_per_pe = static_cast<int>(floor(static_cast<double>(super::get_everything().size()) / CkNumNodes()));
        int pe_ctr=0, nctr=0;
    ...
    void solve_and_evaluate(double kappa_in, CkCallback cb)
    {
        kappa = kappa_in;
        super::solve(kappa);
        //std::cout << CkMyNode() << " FMM timings: " << super::get_timing_info() << std::endl;
        super_charm::thisProxy[CkMyNode()].evaluate(cb);
    }
    ...
    unsigned short level_with_more_nodes_than_procs() const
    {
        ...
        {
            if (super::get_num_nodes_on_level(level) >= CkNumNodes()) 
            {
                return level;
            }
    ...
    void evaluate(CkCallback cb_results)
    {
        ...
        for (typename All::const_iterator it=everything.begin(), end=everything.end(); it != end; ++it)
        {
            ...
            if (  (eval_level < decision_level && (idxer.as_hash_number() % CkNumNodes() == CkMyNode())) 
                  || NodeToProc[idxer] == CkMyNode() ) 
            {
    ...
    void evaluate(EvalMessage *msg)
    {
        ...
        for (int ctr=0; ctr < num_evals; ++ctr)
        {
            ...
            int proc = (eval_level < decision_level) ? idxer.as_hash_number() % CkNumNodes() : NodeToProc[idxer];
            ...
            it != end; ++it)
        {
            ...
            msg_out->cb = CkCallback(CkIndex_VanillaFMM<18,18,300>::recv_data(NULL), CkMyNode(), super_charm::thisProxy);
