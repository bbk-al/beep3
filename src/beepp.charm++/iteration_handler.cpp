...
void IterationHandler::phase_two()
{
    ...
    BEM_FMM_Proxy.solve(beta, beta0, CkSelfCallback(CkIndex_IterationHandler::done_evals()));

    //std::cout << "Evaluating explicit neighbours" << std::endl;
    BEM_FMM_Proxy.near_field_integration(beta, CkSelfCallback(CkIndex_IterationHandler::done_evals()));
...
{
    ++pending;
    assert(pending <= num_workers*2);
    //std::cout << "pending: " << pending << " / " << num_workers << std::endl;
    if (pending == num_workers*2)
    {
