/* Author: david fallaize   */

/*!\file beep_main.cpp
 * \brief The main function for the BEEP solver
 */
#include "beep.h"
#include <vector>
...
int main(int argc, char *argv[]) {
    ...
    generic.add_options()
        ("help", "Display help") 
        ("qual", po::value<int>(&cmdline_qual_points),
		 "set qualocation rule (num pts per triangle on source patches: "
																	"0,1,4,7)")
        ("quad", po::value<int>(&cmdline_quad_points),
		 "set quadrature rule (num pts per triangle on target patches: "
																	"0,1,4,7)")
        ("nbsize", po::value<int>(&cmdline_nbsize),
		 "set BEM neighbourhood size")
        ("kappa", po::value<double>(&cmdline_kappa),
		 "set inverse Debye screening length")
        ("planar", po::value<bool>(&use_planar)->default_value(false),
		 "force use of planar triangles")
		;
    ...
    hidden.add_options()
        ("input-file",
			po::value<std::string>(&config_filename)->default_value(""),
			"xml format configuration input file")
        ;
    ...
    try {
        ...
        BEEP BEM_solver(config, cmdline_quad_points, cmdline_qual_points,
			            cmdline_nbsize, cmdline_kappa, use_planar);
